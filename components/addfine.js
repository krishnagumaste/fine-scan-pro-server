...
export const handleAddFine = async (req,res) => {
    let plate_number = 1;
    ...
    try {
        ...
        const result = await axios.post(`http://` + ip + `/getnumberplate`, temp, {
          headers: {
            'Content-Type': 'application/json', // Set the appropriate Content-Type
          },
        });
        
        if(result.data.number_plate === ''){
            res.status(409).json({
                message: "ML Model failed",
            });
        } else {
            plate_number = result.data.number_plate.replace(/[^a-zA-Z0-9]/g, '');
